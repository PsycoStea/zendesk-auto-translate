...
### v1.0.53 (current)
- **Diagnostic build for the macro autocomplete.** v1.0.52 shipped Phase 4 #13 but the field test surfaced "I created a macro and typing `//` does nothing." Same playbook as the PDF interceptor debugging in v1.0.46: unconditional `[zt-macro]` console logs at every guard in the trigger pipeline. Logs cover: storage load (how many macros came back from `chrome.storage.local`), input event firing on the visible composer, the trigger detection's exact bail reason if any (no selection, range not collapsed, caret not in a text node, walked-back too short, no `//` before the partial, glued to a non-boundary char), menu render (item count + names), and menu positioning (the trigger range's `getBoundingClientRect`). One of these will pinpoint the failure mode. Logs run regardless of `ztDebug` for this version only; will be quieted once root-caused.
...
- **PDF viewer: three errors fixed in one release.** v1.0.48 surfaced a stack of issues that hid behind each other. Working through the console:
  ...
  - **`Failed to fetch` from the content script.** This was the actual blocker. **In Manifest V3, content-script `fetch()` is treated as cross-origin from the chrome-extension origin** — not from the page origin as it was in MV2. So a fetch to `refurbed-merchant.zendesk.com/attachments/...` from the content script became a CORS-restricted cross-origin request without the agent's cookies, and Zendesk's redirector returned an unauthenticated response. Fix: **route the fetch through the background service worker**. Background-SW fetches with `credentials: 'include'` use the user's actual cookie jar for any host in `host_permissions` and bypass CORS entirely. The content script now does `chrome.runtime.sendMessage({ type: 'zt-fetch-pdf', url })`; `background.js`'s handler runs the credentialed fetch and returns the bytes via structured-clone (`ArrayBuffer` over the message channel). Content script then `postMessage`s those bytes to the viewer iframe as before.
- **Diagnostic `[zt-pdf]` logs still in.** Will be quieted in the next release once you confirm a successful PDF render.
